...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'PROVISION_SERVICE', defaultValue: false, description: 'Should the service be provisioned to a Macaw platform runtime?')
        string(name: 'SERVICE_BLUEPRINT_ID', defaultValue:'', description: 'The blueprint id to use for provisioning the service. This will be used if service provisioning is enabled')
        ...
        booleanParam(name: 'SKIP_CERT_VERIFICATION', defaultValue: false, description: 'If set to true then any SSL certificate errors will be ignored. Typically, for self-signed certificates this param can be set to true')
